...
extension URL {
			...
			if nextFD < 0 {
				...
				if wasCreated {
					...
					guard fchmod(nextFD, 0o755) == 0 else {
						close(nextFD)
						throw error("Failed to set permissions of \(component.quoted): \(Errno(rawValue: errno))")
					}
				}
			}
		}
	...
	func secureCloneOrCopy(to destinationURL: Self) throws {
		...
		guard stagingFolderFD >= 0 else {
			throw error("Failed to open \(stagingFolderURL.filePath.quoted): \(Errno(rawValue: errno))")
		}
		...
		guard
			...
			throw error("Failed to move staging file to \(destinationURL.filePath.quoted): \(Errno(rawValue: errno))")
		}
		wasRenamed = true
	}
}
