...
struct CatalogApp {
	...
	fileprivate func with(minimumOSVersion: String) -> Self {
		...
		: {
			...
			jsonObject.fields
			.firstIndex { $0.key == "minimumOSVersion" } // swiftformat:disable:this indent
			.map { jsonObject.fields[$0] = ("minimumOSVersion", .string(.init(minimumOSVersion))) }
			return .init( // swiftformat:disable:previous indent
				adamID: adamID,
...
extension CatalogApp: JSONDecodable {
	fileprivate init(json: JSON.Node) throws {
		...
		jsonObject = object.mappingKeys
		self.json = .init(jsonObject)
	}
}
...
private extension JSON.Object {
	var mappingKeys: Self {
		.init(
			fields
			.map { (.init(rawValue: $0.rawValue.keyMapped), $1.mappingKeys) } // swiftformat:disable:this indent
...
private func getCatalogApps(from url: URL) async throws -> [CatalogApp] {
	...
	return try CatalogAppResults(json: .init(parsingFragment: json)).results
}
