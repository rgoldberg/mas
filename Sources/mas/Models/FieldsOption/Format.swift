...
enum Transform: Hashable { // swiftlint:disable:this one_declaration_per_file
	...
	/// digits, counting from the right; a fractional part & leading `-` sign are
	/// left untouched. See `<group>` in fields-format.md for full semantics.
	...
	static let dateTransformSet = Set([Self.dateOnly, .localTimeZone])
	...
	/// Applies this transform to a string. `dateOnly` / `localTimeZone` are
	/// applied separately, by `DateSpec.formatted(_:)`, since they need the
