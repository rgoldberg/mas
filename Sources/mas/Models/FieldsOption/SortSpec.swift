...
private extension Substring {
	...
	mutating func parseBoundaries(after currentIndex: inout Index) throws(ParsingError) -> SortSpec.Boundaries {
		...
		let boundaryLists =
			try elements.filter { !$0.isEmpty }.map { element throws(ParsingError) in try element.parseBoundaryList() }
