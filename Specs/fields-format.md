...
string-transform           = <capitalize> | <lowercase> | <sentence-case> | <trim-whitespace> | <uppercase> (* type: string *)
number-transform           = <absolute-value> | <round> | <scale> (* type: number *)
number-to-string-transform = <group> (* type: number *)
chronologic-transform      = <iso> | <date-only> | <time-zone> (* type: chronologic *)
...
justify              = <left-justify> | <center-start-justify> | <center-end-justify> | <right-justify> (* default: <left-justify> *)
...
`<strict-coercion>` broadens a matcher's match to also accept a string whose
entire content parses as the matcher's target type, instead of requiring the
value's type to already match (e.g., `%.n` matches `"4.50"`, but not `"a1a"`).
...
`<lenient-coercion>` (`%_n` / `%_N`) broadens `%n` / `%N` further than
`<strict-coercion>`: a string conforms iff it contains exactly 1 number, no
matter what surrounds it (its **trivia**, e.g., a `US$` / `$` prefix or an `MB`
suffix); number transforms may be applied to the number alone, retaining the
trivia.
