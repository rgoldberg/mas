...
A persisted config whose base config name is its own name resolves from the
context above its own, so a config may extend its nearest same-named ancestor.
Any other cyclic chain of references is an error.
